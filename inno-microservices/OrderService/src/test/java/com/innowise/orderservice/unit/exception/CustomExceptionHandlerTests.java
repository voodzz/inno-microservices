...
public class CustomExceptionHandlerTests {
  ...
  void handleNotValidException_ShouldReturnBadRequestWithFieldErrors() {
    ...
    ResponseEntity<Map<String, String>> response =
        exceptionHandler.handleNotValidException(exception);
  ...
  void handleUpdateException_ShouldReturnBadRequestWithErrorAndTimestamp() {
    ...
    ResponseEntity<Map<String, String>> response =
        exceptionHandler.handleUpdateException(exception);

    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
    assertThat(response.getBody()).hasSize(2).containsKey("Error").containsKey("Timestamp");
  ...
  void handleUpdateException_WithIdConstructor_ShouldReturnFormattedMessage() {
    ...
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
    assertThat(response.getBody().get("Error")).isEqualTo("Update failed for entity with id '456'");
  }
  ...
  void handleRetrieveUserException_ShouldReturnInternalServerErrorWithDetails() {
    ...
    ResponseEntity<Map<String, Object>> response =
        exceptionHandler.handleRetrieveUserException(exception);
