 ...
 * @param order_id the ID of the parent order this item belongs to (must not be null)
 * @param item_id the ID of the product being ordered (must not be null)
 * @param quantity the number of units of this item included in the order (must not be null)
 */
public record OrderItemDto(
    Long id,
    @NotNull(message = "Order ID must not be null") Long order_id,
    @NotNull(message = "Item ID must not be null") Long item_id,
